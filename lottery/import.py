...
in_csv = './test.csv'
out_sqlite = './candidate.sqlite'

table_name = 'candidate' # name for the SQLite database table
chunksize = 100000 # number of lines to process at each iteration

# columns that should be read from the CSV file
columns = ['id_no','plate_no','apply_dt','batch','prize_id']
...
for i in range(0, nlines, chunksize):  # change 0 -> 1 if your csv file contains a column header
    ...
    sql.to_sql(df, 
                ...
                index_label='id_no', # use a unique column from DataFrame as index
                if_exists='append') 
cnx.close()    